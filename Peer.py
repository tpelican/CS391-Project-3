...
class Neighbor_Table( dict ):
    ...
    def add( self, peer_name, peer_ip, peer_port ):
        ...
        :param peer_port: the port of the neighboring peer
        :return: n/a
        """
...
class Peer:
    ...
    def connect_to_neighbor( self, peer_ip, peer_port ):
        ...
        try:
            new_neighbor_request = self.create_response( 1, self.name,
                                                         self.ip,
                                                         self.port )
            ...
            self.print_ports( peer_name.decode(), peer_port,
                              str( addr[ 1 ] ), peer_name.decode() )

            # add the neighbor response to the neighbors dictionary
            self.add_neighbor( peer_name.decode(),
                               peer_ip, peer_port )
            self.print_connection_msg( peer_name.decode(), peer_ip,
                                       peer_port )
    ...
    def print_ports( self, name, lookup, ephemeral, message ):
        print( "\n" + self.name + " received " + name + "'s response:" )
